                        ...
                        {meals.filter(k => props[k] && props[k].length)
                            .map(k => {
                                ...
                                return <React.Fragment key={k}>
                                    <TableRow key={`${props.date}_${k}_name`}>
                                        <TableCell colSpan={9}>{k[0].toUpperCase()}{k.slice(1)}</TableCell>
                                    </TableRow>

                                    {props[k].map(({ name, unit }: item) => {
                                        ...
                                        return <TableRow key={`${props.date}_meal_${k}_${name}`}>
                                            <TableCell width={90}></TableCell>
                                            <TableCell width={250}>{name}</TableCell>
                                            <TableCell width={90}>{nutrition.serving}</TableCell>
                                    ...
                                    <TableRow key={`${props.date}_total_${k}`}>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                {tableHeads.map(t => <TableCell align="center" width={90} key={`${props.date}_head_${t}`}>{`${t[0].toUpperCase()}${t.slice(1)}`}</TableCell>)}
                            </TableRow>
                        </TableHead>
            ...
            <Table className={classes.table} aria-label="table">
                <TableHead>
                    <TableRow>
                        ...
                        {tableHeads.map(t => <TableCell key={`tableHead_${t}`} align="center">{`${t[0].toUpperCase()}${t.slice(1)}`}</TableCell>)}
                    </TableRow>
                </TableHead>
